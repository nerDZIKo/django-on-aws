...
# IAM Role for Django application pods
resource "aws_iam_role" "django_pod_role" {
  name = "${var.project_name}-${var.environment}-django-pod-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${local.oidc_issuer_url}:sub" = "system:serviceaccount:default:django-app"
            "${local.oidc_issuer_url}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
# IAM Policy for Django application (S3, Secrets Manager access)
resource "aws_iam_policy" "django_pod_policy" {
  name        = "${var.project_name}-${var.environment}-DjangoPodPolicy"
  description = "IAM policy for Django application pods"
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:s3:::${var.project_name}-${var.environment}-static-files/*",
          "arn:aws:s3:::${var.project_name}-${var.environment}-media-files",
          "arn:aws:s3:::${var.project_name}-${var.environment}-media-files/*"
        ]
      },
      {
...
# Attach the policy to the Django pod role
resource "aws_iam_role_policy_attachment" "django_pod_policy" {
  policy_arn = aws_iam_policy.django_pod_policy.arn
